...
#include "../includes/riscvISA.h"
#include "../includes/basic_simulator.h"
#include "../includes/core.h"
#include "../includes/elfFile.h"
#include "../includes/syscallStructs.h"
...
ac_int<32, true> BasicSimulator::doFstat(const unsigned file, const ac_int<32, false> stataddr)
{
  ...
  std(stataddr + 40, 0);          // unsigned long long
  std(stataddr + 48, filestat.st_size);         // long long
  stw(stataddr + 56, filestat.st_blksize);      // int
  stw(stataddr + 60, 0);          // int
  std(stataddr + 64, filestat.st_blocks);       // long long
  stw(stataddr + 72, filestat.st_atimespec.tv_sec);  // long
  stw(stataddr + 76, filestat.st_atimespec.tv_nsec); // long
  stw(stataddr + 80, filestat.st_mtimespec.tv_sec);  // long
  stw(stataddr + 84, filestat.st_mtimespec.tv_nsec); // long
  stw(stataddr + 88, filestat.st_ctimespec.tv_sec);  // long
  stw(stataddr + 92, filestat.st_ctimespec.tv_nsec); // long
  stw(stataddr + 96, 0);          // long
  stw(stataddr + 100, 0);         // long
