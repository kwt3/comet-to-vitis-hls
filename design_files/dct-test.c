...
int main()
{
  ...
  for (int oneIteration = 0; oneIteration < 512; oneIteration++) {
    ...
    // Compare fast_fixed_output to expected_output
    int pass = 1;
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            if (fast_fixed_output[i][j] != expected_output[i][j]) {
                printf("Mismatch at [%d][%d]: Expected %d, Got %d\n",
                       i, j, expected_output[i][j], fast_fixed_output[i][j]);
                pass = 0;
            }
        }
    }
